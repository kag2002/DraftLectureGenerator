...
from backend.database.session import engine as sqlite_engine, Base
...
def migrate_db():
    ...
    print(f"[MIGRATION] Phat hien dich den PostgreSQL. Tien hanh ket noi...")
    ...
    try:
        ...
        for u in users:
            ...
            if not exists:
                new_u = User(id=u.id, email=u.email, password_hash=u.password_hash, full_name=u.full_name, created_at=u.created_at)
                pg_db.add(new_u)
        pg_db.commit()
        ...
        for c in courses:
            ...
            if not exists:
                new_c = Course(id=c.id, user_id=c.user_id, course_code=c.course_code, course_name=c.course_name, created_at=c.created_at)
                pg_db.add(new_c)
        pg_db.commit()
        ...
        for clo in clos:
            ...
            if not exists:
                new_clo = CLO(id=clo.id, course_id=clo.course_id, clo_code=clo.clo_code, description=clo.description, bloom_level=clo.bloom_level)
                pg_db.add(new_clo)
        pg_db.commit()
        ...
        for ch in chapters:
            ...
            if not exists:
                new_ch = Chapter(id=ch.id, course_id=ch.course_id, sort_order=ch.sort_order, title=ch.title, description=ch.description, created_at=ch.created_at)
                pg_db.add(new_ch)
        pg_db.commit()
        ...
        for mat in materials:
            ...
            if not exists:
                new_mat = ChapterMaterial(id=mat.id, chapter_id=mat.chapter_id, slide_content=mat.slide_content, active_learning_script=mat.active_learning_script, updated_at=mat.updated_at)
                pg_db.add(new_mat)
        pg_db.commit()
